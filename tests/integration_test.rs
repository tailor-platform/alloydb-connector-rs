// Integration tests for alloydb-connector
//
// These tests require a real AlloyDB instance and are automatically skipped
// if the required environment variables are not set.
//
// Use `--show-output` to see skip messages:
//   cargo test --package alloydb-connector --test integration_test -- --show-output
//
// Environment variables:
//   TEST_ALLOYDB_INSTANCE       - Instance URI (projects/PROJECT/locations/REGION/clusters/CLUSTER/instances/INSTANCE)
//   TEST_ALLOYDB_DATABASE       - Database name
//   TEST_ALLOYDB_USER           - Database user
//                                 For IAM auth with service accounts, use truncated form:
//                                 sa@project.iam (not sa@project.iam.gserviceaccount.com)
//   TEST_ALLOYDB_PASSWORD       - Database password (required for static auth, ignored for IAM auth)
//   TEST_ALLOYDB_USE_IAM_AUTH   - Set to "true" to enable IAM authentication
//   TEST_ALLOYDB_IP_TYPE        - IP type: "private" (default) or "public"
//
// Example (static auth):
//   TEST_ALLOYDB_INSTANCE=projects/my-project/locations/us-central1/clusters/my-cluster/instances/primary \
//   TEST_ALLOYDB_DATABASE=postgres \
//   TEST_ALLOYDB_USER=postgres \
//   TEST_ALLOYDB_PASSWORD=secret \
...
//   TEST_ALLOYDB_INSTANCE=projects/my-project/locations/us-central1/clusters/my-cluster/instances/primary \
//   TEST_ALLOYDB_DATABASE=postgres \
//   TEST_ALLOYDB_USER=sa@my-project.iam \
//   TEST_ALLOYDB_USE_IAM_AUTH=true \
//   TEST_ALLOYDB_IP_TYPE=public \
//   cargo test --package alloydb-connector --test integration_test
...
impl TestConfig {
    fn from_env() -> Option<Self> {
        let instance = env::var("TEST_ALLOYDB_INSTANCE").ok()?;
        let database = env::var("TEST_ALLOYDB_DATABASE").ok()?;
        let user = env::var("TEST_ALLOYDB_USER").ok()?;
        let password = env::var("TEST_ALLOYDB_PASSWORD").ok();
        let use_iam_auth = env::var("TEST_ALLOYDB_USE_IAM_AUTH")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
        let ip_type = env::var("TEST_ALLOYDB_IP_TYPE")
            .ok()
...
async fn test_connect_and_query() {
    ...
    let Some(config) = TestConfig::from_env() else {
        println!("SKIPPED: TEST_ALLOYDB_* environment variables not set");
        return;
    };
    ...
    let mut connector_config = AlloyDbConfig::new(&config.instance)
    ...
    let connector = Arc::new(
        AlloyDbConnector::new(connector_config)
            .await
...
async fn test_connector_iam_token() {
    ...
    let connector = AlloyDbConnector::new(connector_config)
        .await
        .expect("failed to create connector");
