use crate::api;
use crate::config::{AlloyDbConfig, IpType};
...
impl CertificateManager {
    pub async fn new(config: &AlloyDbConfig) -> Result<Self, Error> {
        let instance = InstanceUri::parse(&config.instance_uri)?;
        ...
        let cached = build_cached_certificate(&connection_info, &private_key)?;
...
fn build_cached_certificate(
    ...
    let server_ca_certs = parse_pem_certificates(&info.ca_cert)?;

    let mut client_certs = Vec::new();
    for cert_pem in &info.client_cert_chain {
        let certs = parse_pem_certificates(cert_pem)?;
        client_certs.extend(certs);
    }
...
async fn refresh_certificate(
    ...
    build_cached_certificate(&connection_info, &private_key)
}
