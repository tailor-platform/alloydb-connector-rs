...
//!
//! This crate provides a native Rust connector for Google AlloyDB,
//! enabling direct mTLS connections without requiring the AlloyDB Auth Proxy sidecar.
//!
//! ## Usage
//!
//! ```rust,no_run
...
//!     let config_iam = AlloyDbConfig::new(
//!         "projects/my-project/locations/us-central1/clusters/my-cluster/instances/primary"
//!     )?
//!         .with_ip_type(IpType::Private)
//!         .with_iam_auth();
//!
//!     let connector_iam = Arc::new(AlloyDbConnector::new(config_iam).await?);
...
pub use config::{AlloyDbConfig, IpType};
pub use error::Error;
pub use pool::{AlloyDbPool, AlloyDbPoolManager, PooledConnection};
pub use tls::AlloyDbTlsConnector;
...
impl AlloyDbConnector {
    pub async fn new(config: AlloyDbConfig) -> Result<Self, Error> {
    ...
    pub fn make_tls_connector(&self) -> AlloyDbTlsConnector {
        AlloyDbTlsConnector::from_cache(self.cert_manager.get_cache())
    }
    ...
    pub fn create_pool(
        ...
        let manager =
            AlloyDbPoolManager::new(self, dbname, user, password, use_iam_auth, max_lifetime);
        Ok(AlloyDbPool::builder(manager)
            .max_size(max_size)
            .build()
